...
9. The hermes-nix environment does not expose the local browser editor. Do not start or recommend the `serve` command; deliver the PPTD project and PPTX under `/workspace` instead.
10. Do not offer a localhost editor link or `serve` command in the final response.
