...
2. the matching locally generated `.pptx`, with flattened slide images and fade slide transitions applied by default.
...
3. **python3**: run `python3 --version` (on Windows, `python` may be the correct command). Needed for `export_pptx_local.py` / `export_images.py`.
...
2. Visual review with exported page images — **required before PPTX export when the model supports image input (multimodal)**:
   - Run `scripts/export_images.py`. It loads the deck into Kimi's public editor, chooses 导出 → 图片, downloads the images ZIP, unzips it, and stitches all pages into one overview image:
     ...
     ```bash
     python3 ${HERMES_SKILL_DIR}/scripts/export_images.py \
...
4. PPTX conversion: use `scripts/export_pptx_local.py`. It exports the Kimi-rendered page images, packages them as flattened slides in a local presentation, and validates the ZIP/slide structure.
5. Default PPTX options:
   - page transition: `fade` (淡入淡出), written to every slide after local packaging;
   - fonts and other page content are preserved visually in each flattened slide image;
   - the transition may be explicitly disabled with `--transition none`.
6. Export command:

   ```bash
   python3 ${HERMES_SKILL_DIR}/scripts/export_pptx_local.py \
     /abs/path/project/deck.pptd \
...
7. Local export requirements and boundaries:
   ...
   - before browser export, `export_pptx_local.py` uses `export_images.py`, which checks Node.js 18+ and `npm`, then checks `agent-browser --version`; when `agent-browser` is missing or below `0.33.2`, it installs `agent-browser@latest` globally with npm; **PyYAML** is auto-installed with `pip --user` when missing; the image-based visual QA step additionally auto-installs Pillow and websocket-client the same way;
...
8. After export, verify that the output exists and report the generated path. Confirm that every slide has exactly one root-level fade transition in valid CT_Slide order (`cSld`, optional `clrMapOvr`, `transition`, optional `timing/extLst`) and that the PPTX ZIP passes integrity checks. A byte-string search for `<p:fade>` is insufficient because Office ignores transitions nested inside `cSld`. For higher-risk decks, additionally inspect representative rendered/opened pages as appropriate.
9. The hermes-nix environment does not expose the local browser editor. Do not start, recommend, or offer a localhost editor or `serve` command; deliver the PPTD project and PPTX under `/workspace` instead.
10. Element animations (`page.animations` in PPTD — entrance / emphasis / exit / motion-path; see `reference/pptd.md` §6): use them only when the user explicitly requests animations, or when the deck is clearly intended for live presentation / slideshow playback and animation provides a clear benefit for staged disclosure, process demonstration, causal explanation, pacing, visual impact, or brand storytelling. By default, do not add element animations to reading-oriented, self-study, print, or primarily send-and-browse decks. Prefer 1–3 animation groups per page and simple effects such as fade, fly, and zoom. This is separate from the default PPTX slide-level fade page transition written by `export_pptx_local.py`.
11. Speaker notes (`notes` on each `.page`): use them only when the user explicitly requests them; otherwise, do not add them.
12. Parallel tool calls: during PPT production, make tool calls in parallel whenever possible; in each round, write multiple page files in parallel to reduce the number of steps.
